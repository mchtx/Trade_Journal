...
-- CALCULATOR RESULTS Table (Already existed in previous code, but good to include)
...
create policy "Users can view their own calculator results" on calculator_results
  for select using (auth.uid() = user_id);

create policy "Users can insert their own calculator results" on calculator_results
  for insert with check (auth.uid() = user_id);

create policy "Users can update their own calculator results" on calculator_results
  for update using (auth.uid() = user_id);

create policy "Users can delete their own calculator results" on calculator_results
  for delete using (auth.uid() = user_id);
